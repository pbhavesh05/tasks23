...
export function injectPositive(values: number[]): number[] {
    const num = values.findIndex(n => n < 0); 
    if (num === -1){ 
        const total = values.reduce((d,f) => d +f,0); 
        return [...values,total]; 
    }

    const prevSum = values.slice(0,num).reduce((d,f) => d +f,0); 

    return [...values.slice(0,num +1),prevSum, ...values.slice(num+1)];
}
